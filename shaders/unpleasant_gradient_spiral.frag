...
void main(void) {
    float dist_from_center = distance(vec2(0.5), texture_coordinates) * 30.;
    ...
    float spiral_val_normalized = mod(angle / PI + dist_from_center * 0.9, 1.);
    float spiral_val = smoothstep(0., 1., (1. - sin(TAU * spiral_val_normalized - 4. * time)) / 2.);

    vec3 color_out = vec3(spiral_val, 1.1 * off);
